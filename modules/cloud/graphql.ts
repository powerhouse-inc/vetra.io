...
// ---------------------------------------------------------------------------
// Runtime config (vetra-cloud-runtime-config subgraph)
//
// The subgraph stores the user-set overrides as a single env_vars row
// (PH_CONNECT_CONFIG_JSON) and returns:
//   - `effective`: DEFAULT_CONNECT_CONFIG deep-merged with overrides; the full
//     runtime-config envelope shape `{ schemaVersion, packages, packageRegistryUrl,
//     localPackage, connect: { ... } }`.
//   - `overrides`: only what the user has explicitly set, same envelope shape
//     but every key optional.
//
// vetra.to's UI only edits the `connect.*` subtree, so the helpers below
// unwrap `.connect` on read and wrap `{ connect: ... }` on write.
// ---------------------------------------------------------------------------

type RuntimeConfigSubgraphPayload = {
  effective: { connect?: unknown } & Record<string, unknown>
  overrides: { connect?: unknown } & Record<string, unknown>
  schemaVersion: string
  updatedAt: string | null
}

export type RuntimeConfigConnectPayload = {
...
const RUNTIME_CONFIG_FIELDS = `effective overrides schemaVersion updatedAt`

export async function fetchRuntimeConfig(
  tenantId: string,
  token?: string | null,
): Promise<RuntimeConfigConnectPayload> {
  const data = await gql<{ runtimeConfig: RuntimeConfigSubgraphPayload }>(
    `query ($tenantId: String!) {
      runtimeConfig(tenantId: $tenantId) { ${RUNTIME_CONFIG_FIELDS} }
    }`,
    { tenantId },
    token,
  )
  return unwrapConnect(data.runtimeConfig)
}
...
export async function setRuntimeConfig(
  ...
  // Wrap the connect-only overrides into the full envelope the subgraph
  // expects. Empty overrides → empty envelope, which the subgraph treats as
  // "clear all overrides".
  const json: Record<string, unknown> =
    Object.keys(connectOverrides).length === 0 ? {} : { connect: connectOverrides }
  const data = await gql<{ setRuntimeConfig: RuntimeConfigSubgraphPayload }>(
    `mutation ($tenantId: String!, $json: JSON!) {
      setRuntimeConfig(tenantId: $tenantId, json: $json) { ${RUNTIME_CONFIG_FIELDS} }
    }`,
    { tenantId, json },
    token,
  )
  return unwrapConnect(data.setRuntimeConfig)
}

function unwrapConnect(payload: RuntimeConfigSubgraphPayload): RuntimeConfigConnectPayload {
  return {
    effective: (payload.effective?.connect as Record<string, unknown>) ?? {},
    overrides: (payload.overrides?.connect as Record<string, unknown>) ?? {},
